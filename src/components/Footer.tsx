      ...
      <div className="container mx-auto px-6 max-w-7xl">
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-12 mb-16">
          ...
          <div className="space-y-6">
            ...
            <p className="text-gray-600 dark:text-gray-400 text-sm leading-relaxed">
              Empowering Businesses with Modern Technology, Digital Innovation &
              Creative Solutions.
            </p>
            ...
            <div className="flex gap-4">
              {[Briefcase, Camera, Code, MessageSquare].map((Icon, i) => (
                <Link
                  key={i}
                  href="#"
                  className="w-10 h-10 rounded-full bg-gray-100 dark:bg-white/5 flex items-center justify-center text-gray-500 dark:text-gray-400 hover:bg-brand-blue hover:text-white transition-all"
                >
                  <Icon size={18} />
                </Link>
              ))}
          ...
          {/* Quick Links */}
          <div>
            <h4 className="font-heading font-semibold text-gray-900 dark:text-white mb-6">
              Quick Links
            </h4>
            <ul className="space-y-4">
              {[
                ...
                <li key={link.name}>
                  <Link
                    href={link.path}
                    className="text-gray-600 dark:text-gray-400 hover:text-brand-accent text-sm transition-colors"
                  >
                    {link.name}
                  </Link>
                </li>
          ...
          <div>
            ...
            <ul className="space-y-4">
              ...
              <li className="flex items-center gap-3 text-gray-600 dark:text-gray-400 text-sm">
                ...
                <span>{settings?.phone || "+91 9508904653"}</span>
              </li>
              <li className="flex items-center gap-3 text-gray-600 dark:text-gray-400 text-sm">
                <Mail size={18} className="text-brand-accent shrink-0" />
                <span>{settings?.email || "nasiquejahangir000@gmail.com"}</span>
              </li>
              <li className="flex items-center gap-3 text-gray-600 dark:text-gray-400 text-sm">
                <Calendar size={18} className="text-brand-accent shrink-0" />
                <span>{settings?.registrationDate || "21 May 2026"}</span>
              </li>
            </ul>
          </div>
        </div>
        ...
        <div className="pt-8 border-t border-gray-200 dark:border-white/10 flex flex-col md:flex-row justify-between items-center gap-4">
          <p className="text-gray-500 dark:text-gray-500 text-sm text-center md:text-left">
            © 2024 Nooraxis Technologies. All rights reserved.
          </p>
